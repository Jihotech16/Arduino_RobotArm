...
전원을 켜거나 R 키를 누르면 관절 서보 3개와 그리퍼 서보가 모두 90°로 이동합니다.
...
웹의 3D 디지털 트윈은 실제 3MF 출력 부품의 메시를 사용하며, Arduino가 전송하는 `STATE` 값으로 베이스, 연결 관절, 그리퍼 자세를 실시간 시각화합니다. 버튼을 누르면 화면이 먼저 즉시 반응하고 Arduino의 다음 `STATE` 응답으로 실제 명령 상태를 보정합니다. 마우스 드래그로 시점을 회전하고 휠로 확대·축소할 수 있습니다. 현재 구성은 센서 피드백이 아닌 명령 상태 기반 시뮬레이션입니다.
